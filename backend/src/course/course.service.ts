...
export class CourseService {
  ...
  async update(id: string, updateData: UpdateCourseDto): Promise<Course> {
    const updatedCourse = await this.courseModel.findOneAndUpdate({ _id: id }, updateData, { new: true }).exec();
    if (!updatedCourse) {
      throw new NotFoundException(`Course with ID ${id} not found`);
    }
  ...
  async getByInstructor(instructorId: string): Promise<any[]> {
    return this.courseModel
      .find({ instructor_ids: instructorId })
  ...
  async exportAnalytics(req: Request, rating: number): Promise<string> {
    ...
    for (const course of courses) {
      const modules = await this.moduleService.findAllByCourseIntructor(course._id).catch(() => []);
      const ratings = modules.map((module) => {
        if (module.ratings.length === 0) return 0;
  ...
  async updateRating(courseId: string): Promise<Course> {
    ...
    await Promise.all(
      course.modules.map(async (moduleId) => {
        const module = await this.moduleService.findOne(moduleId).catch(() => null);
        if (module && module.ratings.length > 0) {
  ...
  async generateQuiz(course_id: string, moduleId: string, user_id: string): Promise<string> {
    ...
    function getRandomItems(questions: Questions[], n: number, questionDifficulty: string[], type: string) {
      let filter = questions.filter((question) => questionDifficulty.includes(question.difficulty) && type === question.type);
      if (type === "both") filter = questions.filter((question) => questionDifficulty.includes(question.difficulty));
    ...
    else if (progress.average_quiz >= 0.5) questions = getRandomItems(question_bank, module.no_question, ["easy", "medium"], module.type);
    else questions = getRandomItems(question_bank, module.no_question, ["easy"], module.type);
  ...
  async retakeQuiz(quiz_id: string, req: Request): Promise<string> {
    ...
    if (!progress) throw new NotFoundException(`Progress not found for user and course`);
    const moduleIndex = progress.completed_modules.findIndex((item) => item.module_id === module_id);
    if (moduleIndex !== -1) progress.completed_modules.splice(moduleIndex, 1);
    await this.progressService.update(progress._id, progress);
